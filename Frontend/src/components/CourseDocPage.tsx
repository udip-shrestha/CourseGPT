...
export function CourseDocPage({ course }: { course: any }) {
  ...
  const [selectedCanvasIds, setSelectedCanvasIds] = useState<Set<number>>(
    new Set(),
  );
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    const { data, errorMessage } = await documentClient.listDocuments(
      course.id,
      {
  ...
  async function handleUpload() {
    ...
    try {
      for (const file of selectedFiles) {
        const { errorMessage } = await documentClient.uploadDocument(
          course.id,
          file,
        );
        if (errorMessage) {
  ...
  async function handleConfirmDelete() {
    ...
    const { errorMessage } = await documentClient.deleteDocument(
      course.id,
      docToDelete,
    );
    if (errorMessage) {
  ...
  async function handleDownload(docId: string, fallbackName: string) {
    const { data, errorMessage } = await documentClient.downloadDocument(
      course.id,
      docId,
    );
  ...
  async function handlePreview(docId: string) {
    const { data, errorMessage } = await documentClient.previewDocument(
      course.id,
      docId,
    );
    ...
    <>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1 mb-4 max-w-sm">
                Upload PDFs or notes to make them available for CourseGPT to
                reference.
              </p>
            </div>
          ) : (
      ...
      <Dialog open={isAddCanvasOpen} onOpenChange={setIsAddCanvasOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="py-3">
            {loading ? (
              ...
              <p className="text-sm text-muted-foreground">
                No files found in Canvas for this course.
              </p>
            ) : (
          ...
          <div className="flex justify-end gap-2 mt-4">
            <Button variant="outline" onClick={() => setIsAddCanvasOpen(false)}>
              Cancel
            </Button>
            <Button
              onClick={async () => {
                if (selectedCanvasIds.size === 0) {
                  setUploadError(
                    "Please select at least one canvas file to import.",
                  );
                  return;
                }
                ...
                try {
                  ...
                  for (const f of canvasFiles.filter((cf) =>
                    selectedCanvasIds.has(cf.id),
                  )) {
                    try {
                      ...
                      const fileName =
                        f.display_name ||
                        decodeURIComponent(f.filename || "file");
                      const file = new File([blob], fileName, {
                        type: blob.type || "application/octet-stream",
                      });
                      const { errorMessage } =
                        await documentClient.uploadDocument(course.id, file);
                      if (errorMessage) {
                        setUploadError(
                      ...
                      setUploadError(
                        `Failed to fetch/import file ${f.display_name || f.filename}`,
                      );
                      break;
                    }
                  }
