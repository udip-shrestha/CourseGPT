...
class PostgresConnectionManager:
        ...
        def insert_one(self, sql: str, params: Tuple[Any, ...] = ()) -> Optional[str]:
        ...
        def select_one(self, sql: str, params: Tuple[Any, ...] = ()) -> Optional[Dict[str, Any]]:
            ...
            with self.get_connection() as conn:
                with conn.cursor(row_factory=dict_row) as cur:
                    try:
                        cur.execute(sql, params)
                        # psycopg 3 workaround: explicitly check the result size first
                        if cur.rowcount == 0:
                            return None
                        rows = cur.fetchall()
                        if not rows:
        ...
        def execute(self, sql: str, params: Tuple[Any, ...] = ()) -> int:
            """Execute a non-returning query (UPDATE/DELETE) and return number of affected rows."""
            with self.get_connection() as conn, conn.cursor() as cur:
                try:
                    cur.execute(sql, params)
                    affected_rows = cur.rowcount  # number of rows changed
                    conn.commit()
                    return affected_rows          # return it to the caller
                except Exception as e:
                    conn.rollback()
                    self._log_db_error(e, sql)
                    raise
